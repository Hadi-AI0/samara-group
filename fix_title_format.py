...
'''
    ...
    # Replace title followed by SEO block (with or without newline)
    content = re.sub(
        r'(<title>[^<]*</title>)\s*(<!-- Canonical URL -->.*?<link rel="alternate"[^>]*>\s*<!-- Alternate Language Versions -->.*?<link rel="alternate"[^>]*>)',
        r'\1' + seo_block,
        content,
        flags=re.DOTALL
    )
