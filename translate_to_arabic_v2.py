#!/usr/bin/env python3
"""
Translate English content to Arabic for all HTML pages (v2 - improved).
...
def translate_html_file(filepath, output_filepath=None):
    ...
    # Replace text between tags (but not inside other tags)
    ...
    content = re.sub(r'<meta name="description" content="([^"]+)"', lambda m: f'<meta name="description" content="{replace_desc(m)}"', content)
